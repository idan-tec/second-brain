...
- **Evidence and inference are never written the same way.** Every claim in every file carries one of three labels in front of it: `evidence:` for something observed, quoted, or sourced — **with where it came from** · `inferred:` for anything derived, reasoned to, or assumed · `unknown:` for a gap that was left open. **An unlabelled claim is a defect, and a claim labelled `evidence` without its source is a worse one.** (These are per-claim labels inside the text. They are a different thing from `kind: evidence`, which describes the whole file — a `kind: evidence` note still labels each claim in it.) The reason is not tidiness: a confident inference reads identically to a fact six weeks later, it gets built on, and everything above it inherits the error. **Where a document already reasons from a labelled `inferred:` claim, that whole line of reasoning is `inferred:` too.** Use the owner's language for the three words if the folder is not written in English — the discipline is the three states, not the spelling. (Written in as a law after a real install produced a quarter of a megabyte of research in which inference had been recorded in the voice of finding; three load-bearing "facts" turned out to be guesses, and the owner could not learn from his own material.)
