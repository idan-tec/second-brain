# The board

> One screen. Every branch. **Updated: << YYYY-MM-DD >>**
> State: 🟢 fine · 🟡 needs attention · 🔴 neglected · ⚪ dormant by choice

**No areas here yet, and nothing was created for you to fill in.** No `work`, no `personal`, not even the folder that would hold them. **The first one is born from the first thing that plainly belongs to an area of your life, and you name it.** Until then anything you throw in lands in [`inbox/`](inbox/README.md) — which is the right place for it, not a failure.

<< This paragraph comes out in the same edit that adds the first row. A folder with an area in it must never still be
   telling its owner it has none. The table below replaces it: >>

| Branch | State | Last touched | Holds a goal | Sensitivity |
|---|---|---|---|---|
| << [name](branches/<< name >>/STATE.md) >> | << 🟢 >> | << YYYY-MM-DD >> | << — >> | << open · private · sealed >> |

## Not yet set up

Things this system can do that have not been switched on. **Each line disappears by itself once the thing exists** — there is nothing to tick off.

- **Direction — no cycle set.** Nothing here says what these next months are for, so nothing can be prioritised against anything else. Worth about ten minutes, once there is enough here to have an opinion about.

<< When the owner declines an offer, the line changes rather than vanishing:
   `- **Direction** — declined << date >>. Not offered again.`
   and the reason goes in decisions/DECISIONS.md.
   An agent that re-offers a declined thing every session teaches the owner to stop
   reading this board — and the board is the one thing holding the system together. >>

## What the board is saying right now

<< One short paragraph, once there is anything to say. Name the longest-untouched branch with its number of days.
   Say how many goals are active out of the five allowed, if a cycle exists at all.
   State it flatly — the owner already knows how they feel about it; what they lack is the number. >>

## Rules

- **Maintenance is not failure.** A branch can be 🟢 with no goal at all. Most of a life is maintenance.
- **⚪ dormant is a deliberate choice** recorded in `DECISIONS.md`. **🔴 neglected means nobody chose** — it just happened. Keeping those apart is the whole point of the column.
- 🔴 means *"no information"*, not *"something bad happened."* Say it that way.
- This board does not replace a branch's `STATE.md`. It points at it.
- A new branch gets its row **in the same action that creates it**. A branch that is not on the board does not exist.
- **A `Whose business` column appears here the day one of these branches stops being the owner's** — someone else's product, a relative's business, a client's material. Until that day it would be one word repeated down the page. When it appears, `system/OWNERSHIP.md` appears with it, and the column links there.
